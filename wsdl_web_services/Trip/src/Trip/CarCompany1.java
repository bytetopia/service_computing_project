...
public class CarCompany1 {
	public ArrayList<String> getCar(String city, String type, String drivemode) {
		...
		try {
			...
			String opt = "SELECT * FROM carRent1 " + "WHERE city='" + city + "'";
			if (type != null)
				opt += " AND type='" + type + "'";
			if (drivemode != null)
	...
	public String setOrder(String orderno, Integer c_id) {
		...
		try {
			Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/carRent", "abc", "123456");
			String opt = "INSERT INTO orders1 (orderno, c_id) "
					+ "VALUES ('" + orderno + "'," + c_id + ")";
			Statement stmt = connect.createStatement();
	...
	public String getOrder(String orderno) {
		...
		try {
			Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/carRent", "abc", "123456");
			String opt = "SELECT company,city,type,drivemode,price FROM carRent1,orders1 "
